...
class BanCommand extends Command {
    ...
    async run(interaction, client) {
        ...
        if(!userId) return this.error(interaction, "You must provide a valid user.");

        let reason = args.find(x => x.name === "reason")?.value;
        if(!reason) return this.error(interaction, "You must provide a valid reason.");
        ...
        if(userId === client.user.id) return this.error(interaction, "Can't ban myself.");
        ...
        let banCase = new this.client.schemas.case({
            ...
            reason: reason,
            type: "ban",
            lang: ((member?.roles.cache.get(client.config.roles.english) && member?.roles.cache.get(client.config.roles.english)) ? "both" : (member?.roles.cache.get(client.config.roles.english) ? "en" : (member?.roles.cache.get(client.config.roles.german) ? "de" : null))),
            mod: mod.user.id,
            user: userId
        });
        ...
        let banEmbed = new this.embed()
            .setColor("RED").setTitle("🔨 Ban").setTimestamp()
            .addFields([
                {
                    name: "🚨 Moderator",
        ...
        guild.bans.create(userId, { reason: reason, days: 7 })
            .then(banInfo => {
                let banResponseEmbed = new this.embed()
                    .setDescription(`:airplane: \`${banInfo.user?.tag ?? banInfo.tag ?? banInfo}\` has been **banned**.`)
                    .setColor("GREEN");
        
                return this.response(interaction, banResponseEmbed);
            }).catch(error => {
                client.Logger.error(error);
                return this.error(interaction, "Error: ```js\n{e```".replace("{e", error));
            })
    };
};
