...
module.exports = class extends BaseEvent {
    ...
    async run(client) {
        ...
        let i = 0;
        let statusMessages = ["LISTENING_to /help", "PLAYING_on ServerBlaze.eu", "PLAYING_on BTE-Germany.de"];
        setInterval(() => {
            client.user.setPresence({ activities: [{ name: statusMessages[i]?.split("_")[1], type: statusMessages[i]?.split("_")[0] }], status: 'online', afK: false });
            i++;
        ...
        let nextDate = new Date(new Date()
            .setHours((currentDate.getHours()%2 === 0 ? currentDate.getHours()+2 : currentDate.getHours()+1), 0, 0, 0));
