...
class WarnCommand extends Command {
  ...
  async run(interaction, client) {
    ...
    if (member.roles.cache.get(client.config.roles.team))
      return this.error(interaction, "Du kannst keinen Teamler warnen.");
    ...
    let userWarns = await this.client.schemas.case.find({
      type: "warn",
      user: member.user.id,
      deleted: false,
    });
    ...
    let warnResponseEmbed = new this.embed()
      .setDescription(
        `The user has been **warned**. [\`${userWarns.length}\`. warn]`
      )
      .setColor("GREEN");
