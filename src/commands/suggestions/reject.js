  ...
  async run(interaction, client) {
      ...
      suggUser
        .send({
          embeds: [
            new EmbedBuilder()
              .setDescription(
                `Your suggestion (\`${suggestionId}\`) in ${suggestionMessage.guild.name} was rejected, you can find it [here](${suggestionMessage.url}).`
              )
              .setColor(Colors.Red),
          ],
        })
    ...
    return this.response(
      ...
      new this.embed().setDescription(
        `Suggestion ${suggestionId} was rejected.`
      )
    );
  }
}
