...
module.exports = class extends Command {
    ...
    queryHandler(query) {
        ...
        commands.forEach(command => {
            if(command.help.name.includes(query)) queries.push({ name: ("/"+command.help.name + " - " + command.help.description).substr(0,100), value: `${command.help.name}__${command.help.name}`})
            else {
                command.help.aliases.forEach(alias => {
                    if(alias.includes(query)) {
                        let alr = queries.find(q => q.name.startsWith("/"+command.help.name))
                        if(!alr) queries.push({ name: ("/"+command.help.name + " - " + command.help.description).substr(0, 100), value: `${command.help.name}__${command.help.name}`})
                    } else {
                        let alr = queries.find(q => q.name.startsWith("/"+command.help.name))
                        if(!alr) queries.push({ name: ("/"+command.help.name + " - " + command.help.description).substr(0, 100), value: (`${command.help.name}__${command.help.description}`).substr(0, 100)})
                    }
                })
            }
        })
