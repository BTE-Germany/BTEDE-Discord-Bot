...
class WarnCommand extends Command {
    ...
    async run(interaction, client) {
        ...
        if(member.roles.cache.get(client.config.roles.team)) return this.error(interaction, "You can't kick a staff member.");
        if(mod.roles.highest.position <= member.roles.highest.position) return this.error(interaction, "You can't kick members with higher roles than you.");
        if(member.user.id === mod.user.id) return this.error(interaction, "WARUM!??!?!");
        if(member.user.id === client.user.id || member.user.id === "552530299423293441") return this.error(interaction, "You cant kick this user.");
