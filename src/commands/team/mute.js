...
class MuteCommand extends Command {
    ...
    async run(interaction, client) {
        ...
        let member = args.find(x => x.name === "user")?.member || await interaction.guild.members.fetch(args[0]?.value).catch(e => {}) || args[0]?.value;
        ...
        if(!duration || !ms(duration) || isNaN(ms(duration))) return this.error(interaction, "You must provide a valid duration.")

        if(member?.roles || member?.user) {
            if(member.roles.cache.get(client.config.roles.team)) return this.error(interaction, "You can't mute staff members.");
            if(mod.roles.highest.position <= member.roles.highest.position) return this.error(interaction, "You can't mute users with higher roles than you.");
            if(member.user.id === mod.user.id) return this.error(interaction, "WARUM!??!?!");
            if(member.user.id === client.user.id) return this.error(interaction, "This user can't be muted.");
            if(member.roles.cache.get(client.config.roles.muted)) return this.error(interaction, "The user is already muted.")
        }
            ...
            .addFields([
                {
                    ...
                    name: "📌 End",
                    value: `<t:${Math.round(endTime/1000)}:R> | <t:${Math.round(endTime/1000)}:f>`,
                    inline: true
                }
            ])
        ...
        let muteCase = new this.client.schemas.case({
            ...
            reason: reason,
            type: "mute",
            lang: (member?.roles ? ((member.roles.cache.get(client.config.roles.english) && member.roles.cache.get(client.config.roles.english)) ? "both" : (member.roles.cache.get(client.config.roles.english) ? "en" : "de")) : "de"),
            mod: mod.user.id,
        ...
        if(member?.roles || member?.user) member.send({ embeds: [ muteEmbed ] }).catch(e => {});
