const { MessageEmbed, AutocompleteInteraction, CommandInteraction, MessageActionRow, MessageButton } = require("discord.js");
...
module.exports = class extends BaseEvent {
    ...
    async run(client, interaction) {
        if(interaction.type === "APPLICATION_COMMAND") {
            ...
            if(!command.config.userAvailable && !(interaction.member.roles.cache.get(client.config.roles.team) || interaction.member.roles.cache.get("849965829789450250"))) return await interaction.reply({ ephemeral: true, embeds: [ new MessageEmbed().setColor("#ff0000").setDescription(":x: You don't have the required permissions to use this command.")]})
            ...
            if(interaction.customId === "fdrl_selector") {
                ...
                states.forEach(state => {
                    ...
                    if(!client.config.federalSelect[state]) return client.Logger.warn(`No State Role ID for: "${state}"`, "FEDERAL");
                    if(!roles.get(client.config.federalSelect[state])) return;
                    interaction.member.roles.add(client.config.federalSelect[state]).catch(client.Logger.error);
                })
        ...
        } else if(interaction.isButton() && interaction.customId.startsWith("cp_")) {
            if(!interaction.member.roles.cache.get(client.config.roles.team)) return await interaction.deferUpdate().catch(client.Logger.warn);
            let data = client.usersInCreateProcess.find(x => interaction.message.id === x.msg);
            if(!data) return await interaction.deferUpdate().catch(client.Logger.warn);
            if(data.mod != interaction.user.id) return await interaction.reply({ content: `Only the person that started the registration can use this button.`, ephemeral: true }).catch(client.Logger.warn);

            let buttonName = interaction.customId.replace("cp_","");

            if(buttonName === "cancel") {
                client.usersInCreateProcess.delete(data.id);
                await interaction.reply({ content: `:x: Canceled.`, ephemeral: true }).catch(client.Logger.warn);
            ...
            if(buttonName === "finish") {
                for(var d in data) {
                    if(!data[d]) {
                        return interaction.reply({ content: `:x: \`${d}\` is empty.`, ephemeral: true }).catch(client.Logger.warn);
                    };
                };

                if(data.role === "builder") {
                    let channel = client.channels.cache.get(client.config.usersChannel) || await client.channels.fetch(client.config.usersChannel).catch(client.Logger.warn);
                    if(channel) {
                        channel.send({ embeds: [ new MessageEmbed()
                            .setTitle("Neuer Builder")
                            .setDescription(`**Discordname:**\n<@!${data.id}> (${data.tag})\n**Minecraftname:**\n${data.minecraft}\n**Bauort:**\n${data.city}\n**Bundesland:**\n${data.federalState}\n**Koordinaten:**\n${data.coordinates}`)
                            .setTimestamp()
                        ], content: "<@!"+client.config.federalPings[data.federalState].join("> <@!")+">"});
                    };
                };
                ...
                if(data.role === "builder" || data.role === "trial") {
                    ...
                    let member = interaction.guild.members.cache.get(data.id) || await interaction.guild.members.fetch(data.id).catch(client.Logger.warn);
                    member.setNickname(`${data.minecraft} [${data.city}, ${data.federalState}]`).catch(client.Logger.warn);
                    member.roles.add("697198852507435070").catch(client.Logger.warn);
                    if(data.role === "builder") {
                        member.roles.add("692841045901312081").catch(client.Logger.warn);
                        if(member.roles.cache.get("703172621792968775")) member.roles.remove("703172621792968775").catch(client.Logger.warn);
            ...
            if(buttonName != "platform") {
                selectRow.forEach(async row => {
                    row.components.forEach(component => {
                        ...
                        } else if(component.style === "PRIMARY") component.setStyle("SECONDARY");
                    });
                });
            }
