const { MessageEmbed, Interaction, Client, CommandInteraction, SnowflakeUtil } = require('discord.js');
...
class UnmuteCommand extends Command {
    ...
    async run(interaction, client) {
        ...
        if(!memberId) return this.error(interaction, "Please provide a valid user.");

        let guild = interaction.guild;
        let member = guild.members.cache.get(memberId) || await guild.members.fetch(memberId).catch(e => {}) || memberId;
        ...
        let c = await client.schemas.case.findOne({ user: memberId, deleted: { $ne: true }, type: "mute" })

        if(member?.roles) {
            if(c.lang === "en") await member.roles.add(client.config.roles.english).catch(client.Logger.error);
            if(c.lang === "de") await member.roles.add(client.config.roles.german).catch(client.Logger.error);
            if(c.lang === "both") {
                await member.roles.add(client.config.roles.english).catch(client.Logger.error);
                await member.roles.add(client.config.roles.german).catch(client.Logger.error);
            };
            await member.roles.add(client.config.roles.rules)
            await member.roles.remove(client.config.roles.muted).catch(client.Logger.error);
        }
        ...
        if(member.roles) await member.send({ embeds: [ unmuteEmbed ]}).catch(e => {});
