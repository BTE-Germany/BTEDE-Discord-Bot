...
class CaseCommand extends Command {
    ...
    async run(interaction, client) {
        ...
        if(!caseId) return this.error(interaction, "Please provide a valid case ID!");
        ...
        let caseEmbed = new this.embed()
            .setTitle(`${_case.deleted ? ":x: " : ""}Case ${_case.id}: ${titles[_case.type]}`)
            .setColor(colors[_case.type])
            .setDescription("📅 Date:\n" + (_case.end ? `- End: <t:${Math.round(_case.end/1000)}:R> | <t:${Math.round(_case.end/1000)}:f>\n\n- Given: <t:${Math.round(_case.timestamp/1000)}:R> | <t:${Math.round(_case.timestamp/1000)}:f>\n\n- Duration: \`${client.parseTime(_case.end-_case.timestamp)}\`` : `- Given: <t:${Math.round(_case.timestamp/1000)}:R> | <t:${Math.round(_case.timestamp/1000)}:f>`))
            .addFields([
                {
                    name: "🚨 Moderator",
                    ...
                    name: "👤 User",
                    value: `<@!${_case.user}> (${_case.usertag ? `${_case.usertag} | ` : ""}${_case.user})`,
                    inline: true
                }
            ]);
