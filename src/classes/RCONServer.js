...
class RCONServer {
    ...
    async connect() {
        ...
        this._client = new rconClient(this.host, { port: this.port, password: this.password, timeout: this.timeout });
        try {
    ...
    async run(command, noEnd = false) {
        ...
        if(!this._client) return new TypeError("You have to connect the RCON Server first.");
